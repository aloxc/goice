...
func NewGPool(config *PoolConfig) (Pool, error) {
	...
	p := &gPool{
		...
		poolConfig:     config,
		idleConns:      config.InitCap,
		remainingSpace: make(chan bool, config.MaxCap),
	}
...
func (p *gPool) Get() (net.Conn, error) {
	...
	default:
		...
		if p.createNum > p.poolConfig.MaxCap {
			return nil, errors.New("已使用连接已经等于MaxCap了，无法再创建更多的连接")
		}
		conn, err := factory()
		p.removeRemainingSpace()
